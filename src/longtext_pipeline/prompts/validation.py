...
def validate_prompt_template(prompt_content: str) -> ValidationResult:
    ...
    if not prompt_content or not prompt_content.strip():
        ...
        return ValidationResult(
            is_valid=False, issues=issues, template_content=prompt_content
        )
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if len(open_bracket_positions) != len(close_bracket_positions):
            ...
            if len(open_bracket_positions) > len(close_bracket_positions):
                ...
                unclosed_count = len(open_bracket_positions) - len(
                    close_bracket_positions
                )
                if unclosed_count > 0:
                            ...
                            if line.strip() and not line.rstrip().endswith(","):
                                issues.append(
                                    ValidationIssue(
                                        ...
                                        message="Unclosed opening bracket {{ - no matching closing bracket found",
                                        severity="error",
                                    )
                                )
        ...
        for match in re.finditer(single_brace_pattern, line):
            ...
            is_single_open = (
                ...
                and (pos + 1 >= len(line) or line[pos + 1] != "{")
            )
            is_single_close = (
                line[pos] == "}"
                and (pos == 0 or line[pos - 1] != "}")
            ...
            if is_single_open or is_single_close:
                issues.append(
                    ValidationIssue(
                        ...
                        message="Invalid single brace - use {{ }} for template variables",
                        severity="error",
                    )
                )
...
def validate_prompt_file(file_path: str) -> ValidationResult:
    ...
    if not path.exists():
        ...
        return ValidationResult(is_valid=False, issues=issues, file_path=file_path)
...
def validate_required_variables(
    prompt_content: str, required_vars: List[str]
    ...
    for required_var in required_vars:
        if required_var not in found_vars:
            ...
            similar = [
                ...
                or v.lower() in required_var.lower()
            ]

            message = f"Required variable {{{{{required_var}}}}} not found in template"
            if similar:
                message += (
                    f". Did you mean: {', '.join([f'{{{{{s}}}}}' for s in similar])}?"
                )
...
class TemplateValidator:
    ...
    def validate_file(
        ...
        if required_vars and result.template_content:
            var_issues = validate_required_variables(
                result.template_content, required_vars
            )
    ...
    def validate_content(
        self, prompt_content: str, required_vars: Optional[List[str]] = None
    ) -> ValidationResult:
