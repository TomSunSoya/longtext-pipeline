...
def clean_text(content: str, strip_extra_whitespace: bool = True, normalize_encoding: bool = True) -> str:
    """
    ...
    if strip_extra_whitespace:
        ...
        if cleaned_lines:
            ...
            first_idx = 0
            while first_idx < len(cleaned_lines) and not cleaned_lines[first_idx].strip():
                first_idx += 1
...
def extract_sections(content: str, delimiter: str = r'^(#{1,6})\s+(.+)$', 
                     include_headers: bool = True, preserve_structure: bool = True) -> List[str]:
    """
...
def preserve_structure(content: str, encoding_handling: bool = True) -> str:
    ...
    cleaned = clean_text(content, strip_extra_whitespace=False, normalize_encoding=encoding_handling)
    return cleaned
