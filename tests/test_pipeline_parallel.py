...
class TestOrderPreservation:
    ...
    async def test_summarize_order_preservation_with_variable_delays(self):
        ...
        mock_client = MockLLMClient(
            response_sequence=[f"Summary {i}" for i in range(5)], delay=0
        )
        ...
        with patch.object(stage, "_load_prompt_template", return_value="Summarize: "):
            with patch(
                ...
                with patch.object(
                    stage,
                    "_save_summary",
                    side_effect=lambda s, d: f"summary_{s.part_index:02d}.md",
                ):
        ...
        for i, summary in enumerate(summaries):
            assert summary.part_index == i, (
                f"Expected part_index {i} but got {summary.part_index}"
            )
    ...
    async def test_stage_synthesis_order_preservation(self):
        ...
        with patch.object(stage, "_load_prompt_template", return_value="Synthesize: "):
            with patch(
                ...
                with patch.object(
                    stage,
                    "_save_stage_summary",
                    side_effect=lambda s, d: f"stage_{s.stage_index:02d}.md",
                ):
        ...
        for i, stage_summary in enumerate(stage_summaries):
            assert stage_summary.stage_index == i, (
                f"Expected stage_index {i} but got {stage_summary.stage_index}"
            )
...
class TestSemaphoreConcurrency:
    ...
    async def test_stage_synthesis_respects_max_workers_config(self):
        ...
        summaries = [
            Summary(part_index=i, content=f"Summary {i}", metadata={}) for i in range(9)
        ]
...
class TestErrorIsolation:
    ...
    async def test_summarize_error_isolation_single_failure(self):
        ...
        assert (
            len([s for s in summaries if s.part_index != fail_index]) == total_parts - 1
        )
...
class TestMockClientBehavior:
    ...
    async def test_mock_client_fail_indices(self):
        ...
        client = MockLLMClient(response_sequence=["Success"] * 5, fail_indices={1, 3})
