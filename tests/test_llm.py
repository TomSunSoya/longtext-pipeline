...
class TestGetLLMClient:
    ...
    def test_factory_passes_config_values_correctly(self):
        ...
        with patch.object(
            OpenAICompatibleClient, "__init__", return_value=None
        ) as mock_init:
            try:
...
class TestOpenAICompatibleClient:
    ...
    def test_make_request_disables_env_proxy_resolution(self):
        ...
        with patch(
            "src.longtext_pipeline.llm.openai_compatible.httpx.Client"
        ) as mock_client_cls:
...
class TestRetryLLMCall:
    ...
    def test_retry_with_jitter(self):
        ...
        @retry_llm_call(
            max_retries=2, backoff_factor=2.0, add_jitter=True, initial_delay=0.01
        )
        def flaky_function():
            nonlocal call_count
            call_count += 1
    ...
    def test_max_delay_cap(self):
        ...
        @retry_llm_call(
            max_retries=5, backoff_factor=2.0, max_delay=0.05, add_jitter=False
        )
        def flaky_function():
            nonlocal call_count
            call_count += 1
