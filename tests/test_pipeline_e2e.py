...
from src.longtext_pipeline.errors import LLMError, StageFailedError
...
class TestSummarizeStageEndToEnd:
    ...
    async def test_summarize_full_stage_parallel_execution(self, tmp_path):
        ...
        assert len(summaries) == num_parts, f"Expected {num_parts} summaries, got {len(summaries)}"
        
        # Verify order preservation
        for i, summary in enumerate(summaries):
            assert summary.part_index == i, f"Expected part_index {i}, got {summary.part_index}"
    ...
    async def test_summarize_parallel_acceleration(self, tmp_path):
        ...
        expected_parallel_time = (num_parts // 4 + 1) * 0.05
    ...
    async def test_stage_synthesis_full_stage_parallel_execution(self, tmp_path):
        ...
        with patch.object(stage, '_load_prompt_template', return_value="Synthesize: "):
            with patch('src.longtext_pipeline.pipeline.stage_synthesis.get_llm_client', return_value=mock_client):
                with patch.object(stage, '_save_stage_summary', side_effect=lambda s, d: str(d / f"stage_{s.stage_index:03d}.md")):
...
class TestSyncAsyncConsistency:
    ...
    async def test_sync_async_summarize_same_data(self, tmp_path):
        ...
        mock_client_async = MockLLMClient(response_sequence=mock_responses.copy(), delay=0)
        
        with patch.object(stage_async, '_load_prompt_template', return_value="Summarize: "):
            with patch('src.longtext_pipeline.pipeline.summarize.get_llm_client', return_value=mock_client_async):
                with patch.object(stage_async, '_save_summary', side_effect=lambda s, d: str(d / f"summary_{s.part_index:03d}.md")):
                    summaries_async = await stage_async.run(
                        parts=parts.copy(),
                        config={'model': {'name': 'gpt-4o-mini'}, 'pipeline': {'max_workers': 4}},
...
class TestEndToEndIntegration:
    ...
    async def test_full_pipeline_summarize_then_stage_synthesis(self, tmp_path):
        ...
        num_parts = 8
        parts = [
            Part(index=i, content=f"Detailed content for part {i}.", token_count=150, metadata={})
            for i in range(num_parts)
        ]
        ...
        summary_responses = [f"# Summary {i}\n\nKey points from part {i}." for i in range(num_parts)]
        summarize_client = MockLLMClient(response_sequence=summary_responses, delay=0)
        
        with patch.object(summarize_stage, '_load_prompt_template', return_value="Summarize: "):
            with patch('src.longtext_pipeline.pipeline.summarize.get_llm_client', return_value=summarize_client):
                with patch.object(summarize_stage, '_save_summary', side_effect=lambda s, d: str(d / f"summary_{s.part_index:03d}.md")):
        ...
        with patch.object(synthesis_stage, '_load_prompt_template', return_value="Synthesize: "):
            with patch('src.longtext_pipeline.pipeline.stage_synthesis.get_llm_client', return_value=synthesis_client):
                with patch.object(synthesis_stage, '_save_stage_summary', side_effect=lambda s, d: str(d / f"stage_{s.stage_index:03d}.md")):
...
class TestErrorHandling:
    ...
    async def test_summarize_continue_with_partial_on_errors(self, tmp_path):
        ...
        mock_responses = [f"Summary {i}" for i in range(num_parts)]
        mock_client = MockLLMClient(response_sequence=mock_responses, delay=0, fail_indices=fail_indices)
    ...
    async def test_stage_synthesis_continue_with_partial_on_errors(self, tmp_path):
        ...
        num_groups = num_summaries // group_size
        mock_responses = [f"Stage {i}" for i in range(num_groups)]
        mock_client = MockLLMClient(response_sequence=mock_responses, delay=0, fail_indices=fail_group_indices)
