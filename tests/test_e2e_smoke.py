...
class TestCLICommands:
    ...
    def test_cli_run_command(self, runner, smoke_input_file):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch(
                "src.longtext_pipeline.cli.LongtextPipeline.run",
    ...
    def test_cli_run_agent_count_enables_multi_perspective(
        self, runner, smoke_input_file
    ):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch(
                ...
                result = runner.invoke(
                    app, ["run", smoke_input_file, "--agent-count", "2"]
                )
    ...
    def test_cli_run_max_workers_passes_runtime_override(
        self, runner, smoke_input_file
    ):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch(
                ...
                result = runner.invoke(
                    app, ["run", smoke_input_file, "--max-workers", "256"]
                )
    ...
    def test_cli_status_command(
        self, runner, smoke_input_file, mock_llm_responses, temp_dir
    ):
    ...
    def test_cli_status_no_manifest(self, runner, temp_dir):
        ...
        assert (
            ...
            or "no analysis" in result.output.lower()
            or "no manifest" in result.output.lower()
            or "pending" in result.output.lower()
        )
...
class TestManifestManagement:
    ...
    def test_manifest_created_at_pipeline_start(
    ...
    def test_manifest_updated_after_stages(
        self, smoke_input_file, mock_llm_responses, temp_dir
    ):
    ...
    def test_manifest_persisted_to_disk(
        self, smoke_input_file, mock_llm_responses, temp_dir
    ):
...
class TestOutputFileValidation:
    ...
    def test_final_analysis_output(
        self, smoke_input_file, mock_llm_responses, temp_dir
    ):
