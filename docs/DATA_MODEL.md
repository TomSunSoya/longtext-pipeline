...
Final Analysis → final_analysis.md
      ↓
Audit → manifest stats + structured audit result payload
```
...
```json
{
  ...
  "stages": {
    ...
    "audit": {
      "status": "string ('not_started', 'skipped', 'running', 'successful', 'successful_with_warnings', 'failed')",
      "completed_at": "string [optional], timestamp",
      "error": "string [optional], error message",
      "stats": {
        "total_claims": "integer [optional], claims checked during hallucination detection",
        "verified_claims": "integer [optional], claims supported by source material",
        "hallucinated_claims": "integer [optional], unsupported or weakly supported claims",
        "confidence_score": "number [optional], aggregate confidence metric",
        "timeline_anomalies": "integer [optional], temporal inconsistencies found",
        "quality_scoring": "object [optional], composite quality metrics"
      }
    }
  }
}
...
## 6. Audit Result Model

The current runtime does not guarantee a standalone `audit_*.md` artifact.

Instead, the audit stage primarily surfaces its output through:

- `manifest.json` stage stats under `stages.audit`
- the in-memory audit result payload returned by the pipeline
- an optional human-readable detailed report string embedded in that payload

### Current payload shape

Audit results can include fields such as:
...
The exact nested shape may evolve more quickly than the manifest schema, so downstream integrations should treat the manifest as the stable checkpoint contract and the full audit payload as runtime detail.
...
├── summary_02.md
├── stage_01.md                   # Stage synthesis output
├── stage_02.md
├── final_analysis.md             # Cross-stage synthesis
├── metrics.prom                  # Prometheus export
└── .locks/                       # Per-input run locks
```

When `output.dir` is configured in the standard single-file pipeline:

- stage artifacts and `metrics.prom` move to `<output.dir>/.longtext/`
- `manifest.json` and `.locks/` remain in the input-local `.longtext/`

Numbered files use zero-padded numbering.
