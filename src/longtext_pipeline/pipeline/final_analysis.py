...
class FinalAnalysisStage:
    ...
    def _get_specialist_concurrency_limit(
        ...
        requested_limit = config.get("pipeline", {}).get("max_workers", len(selected_specialists))
    ...
    async def _run_multi_perspective(
        ...
        specialist_concurrency = self._get_specialist_concurrency_limit(config, selected_specialists)
        semaphore = asyncio.Semaphore(specialist_concurrency)

        async def run_specialist(analyst_type: str, client, model: str) -> Dict[str, Any]:
...
"""
    ...
    async def run(
        ...
        if mode == "relationship":
            logger.info("[FinalAnalysisStage] EXPERIMENTAL MODE: Relationship-focused analysis enabled")
        if multi_perspective:
            logger.info("[FinalAnalysisStage] MULTI-PERSPECTIVE MODE: Using parallel specialist agents")
        ...
        try:
            ...
            logger.error("[FinalAnalysisStage] LLM error during final analysis: %s", e, exc_info=True)
