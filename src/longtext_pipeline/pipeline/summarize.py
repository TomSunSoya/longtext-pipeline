...
"""
    ...
    async def run(
        ...
        if mode == "relationship":
            logger.warning("Relationship-focused summarize mode is experimental")
        ...
        async def process_part(part: Part):
            async with semaphore:  # Limit concurrent execution
                ...
                try:
                    logger.debug("Processing part %s", part.index)
                    ...
                    logger.info("Completed part %s: %s", part.index, summary_path)
                    ...
                    logger.warning("Failed part %s: %s", part.index, e)
                    ...
                    logger.exception("Unexpected error on part %s", part.index)
                    return None, "", error_info
        ...
        successful = len(
            [
                s
                for s in summaries
                if not s.metadata.get("skipped", False)
                and s.content != "[Skipped - tiny input]"
            ]
        )
        ...
        if successful == 0 and failed == total_parts:
            ...
            self.manifest_manager.update_stage(
                manifest, "summarize", stage_status, error=error_message, stats=stats
            )
            ...
            logger.warning(
                "Summarize stage partial success: %s/%s successful, %s failed",
                successful,
                total_parts,
                failed,
            )
            self.manifest_manager.update_stage(
                manifest, "summarize", stage_status, stats=stats
            )
        else:
            # All succeeded
