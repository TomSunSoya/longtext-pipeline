...
class TestAsyncPerformance:
    ...
    async def test_async_parallel_vs_sequential_summary(self, tmp_path):
        ...
        expected_parallel_time = (num_parts / config["pipeline"]["max_workers"]) * (latency_ms / 1000.0)
...
class TestMultiAgentParallel:
    ...
    async def test_specialist_agents_parallel(self):
        ...
        print(f"  Sequential time (theoretical): 400ms")
        ...
        assert total_time < 0.2, f"Parallel execution too slow: {total_time*1000:.0f}ms"
...
def calculate_expected_speedup(num_items: int, concurrency: int, latency_ms: float) -> dict:
