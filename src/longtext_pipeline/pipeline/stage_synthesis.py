...
class StageSynthesisStage:
    ...
    def _save_stage_summary(self, stage_summary: StageSummary, output_dir: Path) -> str:
        ...
        timestamp = stage_summary.metadata.get(
    ...
    async def run(
        ...
        if not groups:
            ...
            self.manifest_manager.update_stage(
                manifest, "stage", "successful", stats=stats
            )
        ...
        if successful == 0 and failed == total_groups:
            ...
            self.manifest_manager.update_stage(
                manifest, "stage", stage_status, error=error_message, stats=stats
            )
            ...
            self.manifest_manager.update_stage(
                manifest, "stage", stage_status, stats=stats
            )
        else:
            # All succeeded
            ...
            async with semaphore:
                try:
                    ...
                    error_info = {
                        "index": group_index,
                        "summary_files": [
                            f"summary_{s.part_index:02d}.md" for s in group
                        ],
                        "error": str(e),
                    }
